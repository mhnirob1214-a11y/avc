...
def send_telegram(date_str, num, msg, otp, source="", is_update=False):
    ...
    # Telegram inline_keyboard ঠিকভাবে সেট করা
    keyboard = []
    if otp != "N/A":
        keyboard.append([{"text": f"📋 Copy OTP: {otp}", "callback_data": f"copy_{otp}"}])

    # BOT এবং Admin বাটন সবসময় প্রদর্শিত হবে
    keyboard.append([
            ...
            try:
                ...
                if valid_rows:
                    ...
                    if is_first_scan:
                        ...
                        source = "CLI/API Source Here"  # CLI বা API থেকে নিতে হবে
                        tg = send_telegram(latest['date'], latest['num'], latest['sms'], otp, source)
                        ...
                        for item in reversed(valid_rows):
                            ...
                            source = "CLI/API Source Here"  # CLI/API থেকে নিতে হবে

                            if uid not in sent_msgs:
                                tg = send_telegram(item['date'], item['num'], item['sms'], otp, source, is_update=False)
                                ...
                                tg = send_telegram(item['date'], item['num'], f"{item['sms']} (Update)", otp, source, is_update=True)
